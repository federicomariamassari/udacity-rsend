...
Because SLAM is both a CPU and memory -intensive task, I designed a smaller, feature-rich home that would allow for fast, stable mapping resulting in a moderately-sized database. The outcome is overall very pleasing [Figure 1].
...
* Remove `args="--delete_db_on_start"` from the `rtabmap` node to load a pre-existing map of the environment in RViz;
* Remove SLAM-related parameter `Mem/NotLinkedNodesKept`;
